...
# 03/22/2010 ssr moved creation of unit test dataframes to separate functions.
#  3/25/2010 cws Changed diff() calls to dfCompare().
# 09/16/2010 cws Removing hardcoding of NRSA database name, using NRSAdbName
#            instead.
#  4/05/2012 cws Removed calculation of n_w (only done for for boatables), as 
#            this is done in metsChannelMorphology() for both protocols.  Unit
#            test modified accordingly, and rewritten to eliminate production of
#            NAs by coercion, which had the effect of excluding text metric 
#            bangmode from the test.
#  5/29/2014 cws Testing for changes in bangmode only when it is present, which 
#            it is most definitely *not* in wadeable-only datasets.  It's boggling
#            how this error existed for the past 25 months. Sorry about that, Tom.
#
...
# df1<-df1.b; df2<-df2.b; protocols<-prot.b; expected<-exp.b
{
  ...
  exactMets <- 'bangmode'
  if(any(exactMets %in% expected$METRIC)) {
    exactExpected <- subset(expected, METRIC %in% exactMets)
    exactResults <- subset(expected, METRIC %in% exactMets)

    errs <- dfCompare(exactExpected, exactResults, by=c('UID','METRIC'))
    checkEquals(NULL, errs,"Error: bankMorphology text metrics are broken")
  }
  
  almostExpected <- subset(expected, METRIC %nin% exactMets)
  almostResults <- subset(expected, METRIC %nin% exactMets)
  errs <- dfCompare(almostExpected, almostResults, by=c('UID','METRIC'), zeroFudge=1e-10)
  checkEquals(NULL, errs,"Error: bankMorphology numeric metrics are broken")
