...
#          accordingly.
#  2/11/11 cws Changed nrsaphab.assemble to write nrsaphab.csv with metrics
#          names in uppercase.
...
#
{
  if(is.null(flags)) flags <- 'all'
  if(tolower(flags)=='all') {
      # if nothing specified, assume that all available files will be assembled.
      flags <- nrsaphab.allFlags()
  }
  ...
  # Write metrics results to csv file.  Upcase names of metrics along the way.
  nrsaMets$METRIC <- toupper(nrsaMets$METRIC)
  write.csv(nrsaMets, paste(NRSACalcLocation,'nrsaphab.csv', sep='/')
           ,row.names=FALSE
           )
