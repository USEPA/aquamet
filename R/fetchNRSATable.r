...
#  4/14/10 mrc write a unit test for fetchNRSATable
#               exclude datalines for sidechannels when PROTOCOL is BOATABLE
# 11/04/10 cws Handles request for nonexistent table without abrupt termination.
...
{
  # Quick sanity check
  if(toupper(tblName) %nin% toupper(sqlTables(chan)$TABLE_NAME))
      return(sprintf("Table '%s' does not exist in the database", tblName))
