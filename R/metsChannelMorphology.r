...
#            instead.
# 01/11/2011 cws Changed units in metsChannelMorphology.createBankGeometryData
#            from NONE to M.  Changed thalweg PARAMETER WETWID to WETWIDTH
#            to match data in unit test.
# 10/21/2011 cws Modified unit test to use correct parameters DEP_SONR and 
...
# 10/25/2011 cws Ammended above modification as follows: test thalweg data
...
#  6/03/2014 cws Handling NA UNITS values gracefully by translating NA to ""
...

{
  ...
  rc <- writeNRSACalcResults (mets, 'metsChannelMorphology.csv')
     on.exit(metsChannelMorphology.cleanup(chan))
...
#
{
  ...
  df2$SAMPLE_TYPE <- ifelse(df2$UID %in% 1:6, 'PHAB_THALW', 'PHAB_THAL')
  df2$FLAG <- NA
...
#  metstest$RESULT <- ifelse(metstest$UID %in% 1:6 & metstest$METRIC %in% c('xdepth','sddepth')
#                           ,as.character(as.numeric(metstest$RESULT)*100)
#                           ,metstest$RESULT
#                           )
  metstest$RESULT <- with(metstest
                         , ifelse(UID %in% 1:6 & METRIC %in% c('xdepth','sddepth')
                                 ,as.character(as.numeric(metstest$RESULT) * 100)
                          ,ifelse(UID %in% 8:9 & METRIC %in% c('xdepth','sddepth','xwxd','sdwxd')
                                 ,as.character(as.numeric(metstest$RESULT) * 0.3048)
                          ,ifelse(UID %in% 8:9 & METRIC %in% c('xwd_rat','sdwd_rat')
                                 ,as.character(as.numeric(metstest$RESULT) / 0.3048)
                          ,RESULT
                          )))
                         )
