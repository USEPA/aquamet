...
# 10/28/10 ssr Added imageExists arguement and additional test
# 11/09/10 ssr Modified to work with constructImageLocation.r
# 12/15/10 ssr Modified tests.
# 10/25/11 ssr Added explanation of imageExists.

require(RUnit)

constructNRSAValidationResults <- 
  function (df, formMetadata, siteInfo, imageExist)
...
#                 and VISIT_NO.
# imageExist    function to create hyperlink to images (if they exist) using
#                 constructImageLocation as the default.  Set to NULL in 
#                 constructNRSAValidationResults test.
#
# ASSUMPTIONS:
...
#
{
  ...
  NRSASiteKeys <-  names(siteInfo)[names(siteInfo) %in% c('SITE_ID','DATE_COL','VISIT_NO')]

  df <- merge(df, siteInfo[c('UID',NRSASiteKeys)]
  ...
  ## Call constructImageLocation
  
  if (!is.null(imageExist)) {
    df2 <- imageExist(df)
  ...
  imageIfPresent <- names(df)[names(df) %in% 'FORMIMAGE']
  siteIfPresent <- names(df)[names(df) %in% NRSASiteKeys]

  exportColumns <- c('UID',siteIfPresent, tableKeys, 'PARAMETER'
                    ,'TESTDESCRIPTION', 'RESULT'
                    ,unitsIfPresent, flagIfPresent, imageIfPresent
                    ,'COMMENTS'
                    )
  df$COMMENTS <- "                                              "
  df <- df[exportColumns]
...
#  Test .5 not appropriate for constructNRSAValidationResults as format only
#  exists within constructImageLocation
#  constructNRSAValidationResultsTest.5() # Test with one intra site key and FLAG
                                         # and prepared for Open Office Calc.
  constructNRSAValidationResultsTest.6() # Test with one intra site key and FLAG and no image
}
...
# and no UNITS column.
{
  ...
  rr <- constructNRSAValidationResults(baseData, formInfo, sites, imageExist = constructImageLocation)
...
# and no UNITS column, and prepared for Open Office Calc instead of Excel.
{
  ...
  rownames(expected)<-NULL
  rr <- constructNRSAValidationResults(baseData, formInfo, sites, ssFmt='OO', imageExist = constructImageLocation)
  checkIdentical(expected, rr
             ,"Error: Incorrect construction of validation results with one intrasite key formatted for OO Calc"
             )
}

constructNRSAValidationResultsTest.6 <- function()
# Tests constructNRSAValidationResults() with one intrasite key, one data year,
# no UNITS column and no image.
{
  baseData <- data.frame(UID=c('1','1','1','2','2','3')
                        ,TRANLINE=c('A','C','E','B','B','XJ')
                        ,SAMPLE_TYPE='test'
  ...
  expected <- expected[c('UID','SITE_ID','DATE_COL','VISIT_NO','SAMPLE_TYPE'
                        ,'TRANLINE','PARAMETER','TESTDESCRIPTION'
                        ,'RESULT','UNITS','FLAG','COMMENTS'
                        )
                      ]
  rownames(expected)<-NULL
  rr <- constructNRSAValidationResults(baseData, formInfo, sites, imageExist = NULL)
  checkIdentical(expected, rr
             ,"Error: Incorrect construction of validation results with one intrasite key"
             )
}
