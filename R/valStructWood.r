...
#  2/25/2010 cws moved source() calls to NRSAvalidation.r first must open a
#            channel to desired db
#  1/12/2011 cws Modified unit test so expected results from checking realTest
#            data expected TRANSECT E instead of TRANSECT 5.  I don't know what
#            changed in the last few months that would have caused this error.
#
...

{
  ...
  ee <- as.matrix(rbind(
              ...
              ,"Unexpected row count at (UID,TRANSECT,SAMPLE_TYPE,PARAMETER)=(1001,E,PHAB_THALW,WXDSL), n=2"
              ,"Unexpected row count at (UID,TRANSECT,SAMPLE_TYPE,PARAMETER)=(1005,E,PHAB_THALW,WXDSL), n=2"
