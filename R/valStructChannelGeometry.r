...
#  9/10/2010 cws Added check for duplicate key values to vital checks.
#  1/12/2011 cws Updated expected values for realData in unit test, changing
#            expected TRANLINE values from 3 to NONE, to reflect the actual data.
#            Not sure what changed in last few months.
#
...
#
{
  ...
  if(test %in% c('all','vital','synopsis')) {
      ...
      # TRANLINE-LINE.  This takes a while.
      intermediateMessage('.19')
      pp <- stValCountRows(df
      ...
      if(test=='synopsis') {
          probs <- rbind(probs
                        ,sprintf("Duplicate key values count: %d (vital)"
                                ,ifelse(is.null(pp), 0, nrow(pp))
                                )
                        )
      } else {
...
# Tests valStructChannelGeometry()
{
  ...
  checkEquals(NULL,rr
             ,"Error: Detected structure problems where there are none, without WAYPT parameter"
             )
...
,"Unexpected value TRANLINE=(BANK) at (UID,TRANSECT,PARAMETER)=(6,D,SLOPE_ND)" 
,"Unexpected value TRANLINE=(BANK) at (UID,TRANSECT,PARAMETER)=(6,D,SLOPE_ND)" 
,"Unexpected row count at (UID,TRANSECT,SAMPLE_TYPE,PARAMETER,TRANLINE,LINE)=(6,D,PHAB_CHANBFRONT,LATDD_TOP,NONE,1), n=2"
,"Unexpected row count at (UID,TRANSECT,SAMPLE_TYPE,PARAMETER,TRANLINE,LINE)=(6,D,PHAB_CHANBFRONT,LATDD_TOP,NONE,2), n=2"
,"Unexpected row count at (UID,TRANSECT,SAMPLE_TYPE,PARAMETER,TRANLINE,LINE)=(6,D,PHAB_CHANBFRONT,LATDD_TOP,NONE,3), n=2"
,"Unexpected row count at (UID,TRANSECT,SAMPLE_TYPE,PARAMETER,TRANLINE,LINE)=(6,D,PHAB_CHANBFRONT,LATDD_TOP,NONE,4), n=2"
...
,"Absent PARAMETER=(SLOPE) value at UID,TRANSECT=(12,J) "        
  ...
  checkEquals(ee,rr
             ,"Error: Misetected structure problems where they exist"
             )
