...
skipthis<-function() { #####################################################
dd <- first(cc, 'SITE', 'first.SITE')
ee <- subset(dd, first.SITE=='TRUE')
...
ff <- aggregate(list(ltmxcnt=cc$sizen),list(SITE=cc$SITE),count)
gg <- merge(ee,ff)
ltMxMets <- rename(gg, c('DBH','HEIGHT','taxCat','size','DISTANCE'),
             c('ltmxdbh','ltmxht','ltmxspp','ltmxsize','ltmxdist'))
             
ltmxdbh  <- subset(ltMxMets, select=c('SITE','METRIC','ltmxdbh'))
  ltmxdbh$METRIC <- 'ltmxdbh'
  ltmxdbh <- rename(ltmxdbh,'ltmxdbh','VALUE')
ltmxht  <- subset(ltMxMets, select=c('SITE','METRIC','ltmxht'))             
  ltmxht$METRIC <- 'ltmxht'
  ltmxht <- rename(ltmxht,'ltmxht','VALUE')
ltmxspp  <- subset(ltMxMets, select=c('SITE','METRIC','ltmxspp'))
  ltmxspp$METRIC <- 'ltmxspp'
  ltmxspp <- rename(ltmxspp,'ltmxspp','VALUE')
ltmxsize  <- subset(ltMxMets, select=c('SITE','METRIC','ltmxsize'))
  ltmxsize$METRIC <- 'ltmxsize'
  ltmxsize <- rename(ltmxsize,'ltmxsize','VALUE')
ltmxdist  <- subset(ltMxMets, select=c('SITE','METRIC','ltmxdist'))
  ltmxdist$METRIC <- 'ltmxdist'
  ltmxdist <- rename(ltmxdist,'ltmxdist','VALUE')
ltmxcnt  <- subset(ltMxMets, select=c('SITE','METRIC','ltmxcnt'))
  ltmxcnt$METRIC <- 'ltmxcnt'
  ltmxcnt <- rename(ltmxcnt,'ltmxcnt','VALUE')
} ############################################################################
