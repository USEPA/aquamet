...
#          instead.
# 11/02/10 cws Modifying to handle single-protocol datasets without choking.
...
#were set to zero.
{
  ...
  riverMets <- subset(metsExpected, UID %in% unique(riverData$UID))

  streamData <- subset(testData, SAMPLE_TYPE=='PHAB_THALW')
  streamMets <- subset(metsExpected, UID %in% unique(streamData$UID))
  ...
  tt <- merge(testDataResult, metsExpected, by=c('UID','METRIC'), all=T)
  ...
  testDataResult<- metsChannelHabitat.1(riverData)
  tt <- merge(testDataResult, riverMets, by=c('UID','METRIC'), all=T)
  tt$diff <- tt$RESULT - tt$EXPECTED
  errs <- subset(tt, abs(diff) > 10^-7 | is.na(RESULT) != is.na(EXPECTED))
  checkEquals(0, nrow(errs)
  ...
  # Check mixed protocol calculations
  testDataResult<- metsChannelHabitat.1(streamData)
  tt <- merge(testDataResult, streamMets, by=c('UID','METRIC'), all=T)
  tt$diff <- tt$RESULT - tt$EXPECTED
  errs <- subset(tt, abs(diff) > 10^-7 | is.na(RESULT) != is.na(EXPECTED))
  checkEquals(0, nrow(errs)
