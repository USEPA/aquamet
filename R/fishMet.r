...
################################################################################
	...
	inCts <- mutate(inCts,FINAL_CT=as.numeric(FINAL_CT),ANOM_CT=as.numeric(ANOM_CT),IS_DISTINCT=as.integer(IS_DISTINCT),TAXA_ID=as.integer(TAXA_ID))
  ...
  if(CALCNAT=='Y'){
	...
	if(length(grep('ANOM_CT',names(inCts2)))>0){
  ...
  if(CALCNAT=='Y'){
    df.empty <- data.frame(expand.grid(SAMPID=unique(merge3$SAMPID),variable=c('TOTLNIND','TOTLNTAX','BENTINVNIND','BENTINVNTAX','BENTINVPIND','BENTINVPTAX','CARNNIND','CARNNTAX','CARNPIND','CARNPTAX','CATONIND','CATONTAX'
  ...
  merge3.long <- melt(merge3,id.vars='SAMPID')  
  ...
  allfishMetrics1 <- merge(merge3.long,df.empty,by=c('SAMPID','variable'),all.y=TRUE)
  ## Melt data frame, keeping missing values, then fill in missing values with 0 as long as it isn't WTD_TV
  allfishMetrics1 <- mutate(allfishMetrics1,value=ifelse(variable %nin% c('WTD_TV','NAT_WTD_TV') & is.na(value),0,value))
  allfishMetrics2 <- dcast(allfishMetrics1,SAMPID~variable)
  if(CALCNAT=='Y'){
	...
	if(nrow(noFish)>0){
		merge4 <- melt(smartbind(allfishMetrics2,noFish),id.vars='SAMPID',na.rm=FALSE)
			merge4 <- mutate(merge4,value=ifelse(variable %nin% c('WTD_TV','NAT_WTD_TV','FLAG') & is.na(value),0,value))
