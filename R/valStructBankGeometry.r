...
# 09/17/10 cws modified call to stValNonNumericValues to remove combinations of
#          SAMPLE_TYPE and PARAMETER (like boatable bank angles) that are expected
#          to find non-numeric values.
# 10/08/10 cws debugged, no lasting changes made
# 10/28/10 cws corrected synopsis descriptions of absent PARAMETER values
#
...
#
{
  ...
  if(test %in% c('all','vital','synopsis')) {
      ...
      pp <- stValNonNumericValues(subset(df
  ...
  if(test %in% c('all','nonvital','synopsis')) {
      ...
      if(test=='synopsis') {
          probs <- rbind(probs
                        ,sprintf("Absent PARAMETER values (streams, TRANSDIR='NONE'): %d"
                                ,ifelse(is.null(pp), 0, nrow(pp))
                                )
                        )
      } else {
      ...
      if(test=='synopsis') {
          probs <- rbind(probs
                        ,sprintf("Absent PARAMETER values (rivers, SAMPLE_TYPE='PHAB_CHANBFRONT'): %d"
                                ,ifelse(is.null(pp), 0, nrow(pp))
                                )
                        )
      } else {
      ...
      if(test=='synopsis') {
          probs <- rbind(probs
                        ,sprintf("Absent PARAMETER values (rivers, SAMPLE_TYPE='PHAB_CHANB'): %d"
                                ,ifelse(is.null(pp), 0, nrow(pp))
                                )
                        )
      } else {
