...
# df1<-df1.b; df2<-df2.b; protocols<-prot.b; expected<-exp.b
{
  ...
  almostExpected$RESULT <- as.numeric(almostExpected$RESULT)
  almostResults <- subset(rr, METRIC %nin% exactMets)
  almostResults$RESULT <- as.numeric(almostResults$RESULT)
  errs <- dfCompare(almostExpected, almostResults, by=c('UID','METRIC'), zeroFudge=1e-7)
