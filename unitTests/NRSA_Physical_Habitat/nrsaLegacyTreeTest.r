...
nrsaLegacyTreeTest <- function()
{
  ...
  testMets <- legMets[order(legMets$SITE, legMets$METRIC),]
  ...
  testMets.1$VALUE <- as.numeric(testMets.1$VALUE)
  sasMets.1$VALUE <- as.numeric(sasMets.1$VALUE)

  aa <- dfCompare(testMets.1, sasMets.1, c('SITE', 'METRIC'), zeroFudge=1e-9)
  checkEquals(NULL, aa, "Error: nrsaLegacyTree numeric metrics are broken")
  bb <- dfCompare(testMets.2, sasMets.2, c('SITE', 'METRIC'))
  checkEquals(NULL, bb, "Error: nrsaLegacyTree character metrics are broken")
...
nrsaLegacyTreeTest.inputData <- function()
...
nrsaLegacyTreeTest.testResults <- function()
