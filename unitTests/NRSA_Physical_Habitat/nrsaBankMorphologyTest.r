...
nrsaBankMorphologyTest.process <- function(df1, protocols, expected)
# The bulk of the unit testing is done here
{
  ...
  if(any(exactMets %in% expected$METRIC)) {
    ...
    exactResults <- subset(rr, METRIC %in% exactMets)

    errs <- dfCompare(exactExpected, exactResults, by=c('SITE','METRIC'))
    checkEquals(NULL, errs,"Error: bankMorphology text metrics are broken")
  }
  
  almostExpected <- subset(expected, METRIC %nin% exactMets)
  almostExpected$VALUE <- as.numeric(almostExpected$VALUE)
  almostResults <- subset(rr, METRIC %nin% exactMets)
  almostResults$VALUE <- as.numeric(almostResults$VALUE)
  errs <- dfCompare(almostExpected, almostResults, by=c('SITE','METRIC'), zeroFudge=1e-7)
...
nrsaBankMorphologyTest.inputData <- function()
...
nrsaBankMorphologyTest.testResults <- function()
