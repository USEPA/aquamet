...
bedstabOut <- nrsaBedStability(bXdepth =  subset(chanmorphOut, METRIC == 'xdepth' & SITE %in% unique(bD$SITE))
                        ...
                        ,lsub_dmm = subset(subcharOut, METRIC == 'lsub_dmm')
                        ,lsub2dmm = subset(subcharOut, METRIC == 'lsub2dmm')
                        ,rp100 =    subset(respoolOut, METRIC == 'rp100')
                        ,v1w_msq =  subset(lwdOut, METRIC == 'v1w_msq')
                        ,xbkf_h =   subset(chanmorphOut, METRIC == 'xbkf_h')
                        ,xbkf_w =   subset(chanmorphOut, METRIC == 'xbkf_w')
                        ,xfc_lwd =  subset(fishCvrOut, METRIC == 'xfc_lwd')
                        ,xslope =   subset(slopeOut, METRIC == 'xslope')
                        ,xwidth =   subset(chanmorphOut, METRIC == 'xwidth')
                        )
