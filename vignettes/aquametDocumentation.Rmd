...
output: 
  rmarkdown::html_document:
    toc: TRUE
vignette: >  
  %\VignetteIndexEntry{aquamet Documentation}
...
You will notice that code blocks are shown throughout the document. Clicking on the arrow next to `Code` above each block will hide it by folding it up. To hide or reveal all of the code blocks, click on the `Code` arrow in the upper right at the very top of the document. If you hover in the upper right corner of each code block, you can copy the code to the clipboard and run it in your own session of R.

# Getting started

First, we need to load the `aquamet` library. You will also want to ensure you have the `tidyr` package installed and loaded because we will use it to make data wider or longer, depending on our needs. There are also a few other packages that we may use for manipulating the data while preparing it for use with the `aquamet` package.
...
## Calculating NLA physical habitat indicators
...
The output of this function provides three variables along with the condition variable. RVegQ represents the observed riparian vegetation complexity and RVegQc3x15 is the site-specific expectation for riparian complexity, based on site-related input variables. RVegQc3OE is the observed/expected ratio used to assign the condition class.
...
Similar to the riparian vegetation complexity indicator output, the observed littoral vegetation complexity (LitCvrQ), the site-specific expected value (LitCvrQc3x15), and the observed/expected ratio (LitCvrQc3OE) are all provided by the \`nlaLitVegCompIndicator\` function. The LITCVR_COND represents the condition class assigned based on the latter value.
...
Dataset Description ------------- ------------------------------------------- nrsaChanconst Channel constraint characteristics for entire site. nrsaChannel Channel characteristics for both boatable and wadeable streams by TRANSECT, including data on bank characteristics, fish cover, large woody debris, littoral substrates, and boatable constraint variables. nrsaChanrip Channel riparian data by TRANSECT and BANK. Includes canopy cover densiometer readings, human influence, visual riparian measurements, and wadeable bank undercut and angle measurements. nrsaChanxsec Channel cross-section measurements by TRANSECT and BANK for wadeable sites. nrsaInvleg Riparian legacy and invasive tree measurements (no longer used). nrsaLittoral Nonwadeable littoral depth measurements by TRANSECT and LINE. nrsaSlope Slope and bearing measurements. nrsaThalweg Thalweg depth measurements by TRANSECT and STATION. nrsaVisits Site and type of site (VALXSITE), as BOATABLE or WADEABLE.\
...
bLatLon <- nrsaChanrip |>
  ...
  dplyr::group_by(SITE) |>
  dplyr::summarise(A = protectedGreatCircleDistance(LAT_A, LON_A, LAT_B, LON_B),
...
bTransectSpacing <- bLatLon |>
  ...
  dplyr::group_by(SITE) |>
  dplyr::mutate(medianDistance = median(DISTANCE, na.rm=T)) |>
  dplyr::ungroup() |>
  dplyr::mutate(correctedDistance = ifelse(abs((DISTANCE-medianDistance)/medianDistance) >0.5, 
...
wNumStationsPerTransect <- filter(nrsaThalweg, VALUE!='' & !is.na(VALUE) & TRANSECT %in% LETTERS[1:11]) |>
  dplyr::group_by(SITE) |>
  dplyr::summarise(lastTransect = max(TRANSECT)) |>
  dplyr::ungroup() |>
  merge(wNumStations, by = 'SITE', all.y=T) |>
  dplyr::mutate(nSta = ifelse(TRANSECT==lastTransect, nSta-1, nSta)) |>
  select(-lastTransect)

wTransectSpacing <- merge(wThalwegIncrements, wNumStationsPerTransect, by = 'SITE') |>
  dplyr::mutate(VALUE = VALUE * nSta) |>
  select(SITE, TRANSECT, VALUE) |>
...
### Combine all NRSA metrics

Now we can combine all of the metrics into a single data frame, which we can subset for purposes of calculating physical habitat indicators. Doing this in the manner below will convert all values to character because some metric values are character strings and some numeric.

```{r combine-nrsa-metrics}
# Combine all metrics and select only those needed
nrsaMetrics <- rbind(bankMorphMetrics, canDensMetrics,
                     chanCharMetrics, chanHabMetrics,
                     chanMorphMetrics, fishCovMetrics,
                     generalMetrics, nrsaHumInflMetrics,
                     largeWoodyDebrisMetrics, littoralDepthMetrics,
                     nrsaRiparianVegMetrics, slopeBearingMetrics,
                     nrsaSubstrateCharMetrics, nrsaSubstrEmbedMetrics,
                     residualPoolMetrics, bedStabilityMetrics)

```

## Calculating NRSA physical habitat indicators

Now that we have all of the NRSA physical habitat metrics calculated, we are ready to calculate indicators. Sometimes, in practice, certain metrics required for physical habitat indicators are missing because of incomplete data collection in the field. These can sometimes be filled in with other metrics that have been determined to be adequate substitutes, but we will not include this in our examples. If the primary metrics required for a particular indicator are missing, that indicator will be assigned Not Assessed.

There are 4 physical habitat indicators for NRSA: the NRSA Instream Cover Indicator (`nrsaInstrmCoverIndicator()`), the NRSA Relative Bed Stability Indicator (`nrsaRelBedStabilityIndicator()`), the NRSA Riparian Vegetation Indicator (`nrsaRiparianVegIndicator()`), and the NRSA Riparian Disturbance Indicator (`nrsaRipDistIndicator()`). As with the NLA indicators, additional site-related variables are required for calculating each of the NRSA indicators except the NRSA Riparian Disturbance Indicator, and these variables vary by indicator. These variables include aggregated ecoregion (the same AG_ECO9 from NLA), protocol (WADEABLE or BOATABLE, based on field methodology used), latitude and longitude in NAD83 and decimal degrees, elevation in meters, and watershed area in square kilometers. As with the example for NLA indicators, we will create a set of example data for demonstration purposes. Unlike the NLA indicators, there is only one primary metric used for each indicator.

We will also create a simplified input data frame that can work for all of the indicator functions. We need the following metrics as inputs to these functions: mean wetted width (xwidth), mean slope (xslope), log of relative bed stability (lrbs_g08), riparian human disturbance (w1_hall), mean areal proportion of natural fish cover (xfc_nat), and mean canopy, mid and ground cover (xcmgw).

```{r nrsa-create-site-variables}
nrsaSitedata <- data.frame(SITE=c(10000, 10002, 13799, 13904),
                           PROTOCOL = c('WADEABLE', 'WADEABLE', 'BOATABLE', 'BOATABLE'),
                           AG_ECO9=c('SPL', 'TPL', 'SAP', 'CPL'),
                           LAT_DD=c(39.01491, 38.19893, 34.95092, 32.15905),
                           LON_DD=c(-98.01046, -95.42768, -87.04203, -87.01008),
                           ELEV=c(403.88, 308.02, 172.05, 31.07),
                           AREA=c(20.9043, 26.7714, 4785.6267, 935.0037))

nrsaSitedata

# Filter metrics for those necessary, convert all to numeric, and pivot wider
nrsaMets_forIndic <- filter(nrsaMetrics, METRIC %in% c('xwidth', 'xslope', 
                                                       'lrbs_g08', 'w1_hall',
                                                       'xfc_nat', 'xcmgw')) |>
  mutate(VALUE = as.numeric(VALUE)) |>
...
indicIn <- merge(nrsaSitedata, nrsaMets_forIndic, by = 'SITE')

```

### NRSA Instream Cover Indicator

This indicator assesses the areal natural fish cover in the stream and is based on a model which predicts the expected instream cover for a site based on the additional variables provided. The required inputs are ecoregion, protocol, latitude, longitude, mean slope, mean wetted width, elevation, and watershed area. The actual variables used in each model vary by ecoregion and protocol. The mean wetted width (xwidth) and mean slope (xslope) are physical habitat metrics calculated using the `nrsaChannelMorphology()` and `nrsaSlopeBearing()` functions, respectively. These two metrics are also necessary for the riparian vegetation and bed stability indicators.

```{r instrm-cover}

instrCoverCond <- nrsaInstrmCoverIndicator(
  x = indicIn,
  sampID = 'SITE', 
  ecoreg = 'AG_ECO9',
  protocol = 'PROTOCOL', 
...
print(instrCoverCond)
```

### NRSA Riparian Vegetation Indicator

This indicator for riparian vegetation condition requires all of the same variables to calculate the indicator and assign condition. As with the NRSA Instream Cover Indicator, different ecoregions require different additional variables to assess condition, and these are detailed in the documentation for the function (use `?nrsaRiparianVegIndicator`).

```{r nrsa-riparian-veg-indic}
nrsaRipVegCond <- nrsaRiparianVegIndicator(
  x = indicIn,
  sampID = 'SITE', 
  ecoreg = 'AG_ECO9',
  protocol = 'PROTOCOL',
...
print(nrsaRipVegCond)
```

### NRSA Riparian Disturbance Indicator

This indicator requires only the w1_hall metric from the `nrsaHumanInfluence()` function. The thresholds to set condition are the same nationwide.

```{r nrsa-rip-dist-indicator}
nrsaRipDistCond <- nrsaRipDistIndicator(
  x = indicIn,
  sampID = 'SITE',
  w1_hall = 'w1_hall'
)

print(nrsaRipDistCond)

```

### NRSA Relative Bed Stability Indicator

This final NRSA physical habitat indicator assesses the condition of the stream based on relative bed stability and is dependent on the ecoregion and protocol. Like the riparian vegetation and instream cover indicators, this indicator requires site-related variables and the mean wetted width and mean slope metrics.

Recall that the relative bed stability metric function used a number of metrics as inputs, so if any critical metrics are missing for the purposes of calculating the lrbs_g08 metric, it will be missing. As mentioned earlier, there are protocols for replacing missing lrbs_g08 values, but since we have that value for all sites in our example, we will not cover this process here.

```{r rel-bed-stability-cond}
nrsaRBSCond <- nrsaRelBedStabilityIndicator(
  x = indicIn,
  sampID = 'SITE', 
  ecoreg = 'AG_ECO9',
  protocol = 'PROTOCOL',
...
We have now examined in detail the various NLA and NRSA physical habitat datasets that are necessary to calculate physical habitat metrics for these two surveys. We have walked through the calculation of metrics using each of the functions in the `aquamet` package, and we have calculated the habitat indicators for NLA and NRSA, based on the calculated metrics. This document is intended to serve as a guide to get from raw data to final indicators, but it does not cover all possible situations where some data might be missing for specific field measurements. When using a new dataset, if errors or other issues are encountered, examining the input datasets will often reveal the source of problems.
