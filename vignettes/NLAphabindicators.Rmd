---
title: "Calculating NLA Habitat Indicators"
...
verticalHeightDrawdown <- subset(nlaPhabEx,PARAMETER=='VERT_HEIGHT_DD', select=-PARAMETER) |> 
  plyr::mutate(VALUE = as.numeric(VALUE))                                                            
...
bfMets.wide <- reshape2::dcast(bfMets, SITE~METRIC, value.var='VALUE') # Can use either reshape2 or data.table package, but input needs to be in data.table format if latter
...
rv.wide <- reshape2::dcast(rv, SITE~METRIC, value.var='VALUE')
...
ss.wide <- reshape2::dcast(ss, SITE~METRIC, value.var='VALUE') |> 
  dplyr::select(SITE, SSFCBEDROCK,SSFCBOULDERS) |>
  plyr::mutate(SSFCBEDROCK=as.numeric(SSFCBEDROCK), SSFCBOULDERS=as.numeric(SSFCBOULDERS))
```
...
drawdown <- subset(nlaPhabEx,PARAMETER=='DRAWDOWN',select=-PARAMETER)
horizontalDistance_dd <- subset(nlaPhabEx,PARAMETER=='HORIZ_DIST_DD',select=-PARAMETER) |>
  plyr::mutate(VALUE = as.numeric(VALUE)) # This one is manually converted to numeric because it is required by the function to be
...
fc.wide <- reshape2::dcast(fc, SITE~METRIC, value.var='VALUE') |>
...
am.wide <- reshape2::dcast(am, SITE~METRIC, value.var='VALUE') |>
  dplyr::select(SITE, AMFCFLOATING, AMFCEMERGENT) |>
  plyr::mutate(AMFCFLOATING = as.numeric(AMFCFLOATING),
